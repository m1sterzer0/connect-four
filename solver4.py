...
class Solver(object) :
    ...
    ## Set up the search order as [3,2,4,1,5,0,6]
    columnOrder = [0] * Position.WIDTH
    for i in range(Position.WIDTH) :
        columnOrder[i] = Position.WIDTH//2 + (1 if i%2 == 0 else -1) * ((i+1) // 2)
...
if __name__ == "__main__" :
    ...
    print("Mean Time: %.3fs   Mean Number Positions Evaluated: %.1f   Positions Per Second: %.1f" % (totaltime/len(lines), 1.0*totalevaluations/len(lines), totalevaluations/totaltime ))
